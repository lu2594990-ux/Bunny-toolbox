jQuery(() => {
    ...
    styleEl.textContent =
        ...
        ".gs-new{display:flex;align-items:center;padding:10px 14px;gap:8px;cursor:pointer;color:#c44569;font-size:12px;}" +
        ".gs-new:active{background:#fff5f8;}" +
        /* 预设 - 筛选栏 */
        ...
        ".pi .pn.on-color{color:#c44569;}" +
        ".pi .pn.off-color{color:#aaa;}" +
    ...
    panel.innerHTML =
        ...
        '<div class="tc on" data-t="search">' +
            '<div class="sh">' +
                ...
                '<div class="epop"></div>' +
            '</div>' +
            '<div class="sb"><div class="tip">选中文字后点🐰自动搜索<br/>输入网址可直接访问</div><iframe sandbox="allow-scripts allow-same-origin allow-forms allow-popups" referrerpolicy="no-referrer" style="display:none;"></iframe><a class="fb" target="_blank" rel="noopener" style="display:none;">加载不出来？点这里 ↗</a></div>' +'</div>' +
        '<div class="tc" data-t="preset">' +
            '<div class="gs">' +
                '<div class="gs-bar"><span class="gs-icon">📋</span><span class="gs-name">全部条目</span><span class="gs-cnt"></span><span class="gs-arr">▼</span></div>' +
    ...
    /* 状态 */
    var currentGroup = null; /* null=全部, 数字=组index */
    var editingGroup = null; /* null=不在编辑, 数字=组index */
    var selectMode = false;/* 多选创建新组 */
    var selected = {};       /* 勾选的id */
    ...
    function renderBar() {
        ...
        if (editingGroup !== null && groups[editingGroup]) {
            ...
            var order = getActiveOrder();
            gsCnt.textContent = order.length + "条";
        }
    }
    ...
    function renderList() {
        ...
        for (var i = 0; i < order.length; i++) {
            (function (entry) {
                ...
                nameEl.className = "pn " + (en ? "on-color" : "off-color");
                ...
                if (selectMode || editingGroup !== null) {
                    ...
                    tog.addEventListener("click", function (e) {
                        ...
                        setTimeout(function () {
                            ...
                            if (nowOn) { tog.classList.add("on"); nameEl.className = "pn on-color"; }
                            else { tog.classList.remove("on"); nameEl.className = "pn off-color"; }
    ...
    function renderActions() {
        ...
        if (editingGroup !== null) {
            /* 编辑组模式 */
            var saveBtn = document.createElement("button"); saveBtn.className = "pbtn green"; saveBtn.textContent = "✅ 保存";
            ...
            var canBtn = document.createElement("button"); canBtn.className = "pbtn gray"; canBtn.textContent = "✖ 取消";
            ...
            /* 多选创建组模式 */
            var savBtn = document.createElement("button"); savBtn.className = "pbtn purple"; savBtn.textContent = "💾 存为组";
            ...
            var onBtn = document.createElement("button"); onBtn.className = "pbtn green"; onBtn.textContent = "🔛 全开";
            onBtn.addEventListener("click", function (e) {
                e.stopPropagation();
                for (var j = 0; j < g.ids.length; j++) { if (!isItemEnabled(g.ids[j])) toggleItem(g.ids[j]); }
                setTimeout(function () { renderList(); pmsgEl.textContent = "✅ 已全部开启"; pmsgEl.style.color = "#27ae60"; }, 100);
            });
            var offBtn = document.createElement("button"); offBtn.className = "pbtn red"; offBtn.textContent = "🔌 全关";
            offBtn.addEventListener("click", function (e) {
                e.stopPropagation();
                for (var j = 0; j < g.ids.length; j++) { if (isItemEnabled(g.ids[j])) toggleItem(g.ids[j]); }
                setTimeout(function () { renderList(); pmsgEl.textContent = "❌ 已全部关闭"; pmsgEl.style.color = "#e74c3c"; }, 100);
            });
            var editBtn = document.createElement("button"); editBtn.className = "pbtn pink"; editBtn.textContent = "✏️ 编辑";
            ...
            var renBtn = document.createElement("button"); renBtn.className = "pbtn gray"; renBtn.textContent = "✏️ 改名";
            renBtn.addEventListener("click", function (e) {
                ...
                pmsgEl.textContent = "✅ 已改名"; pmsgEl.style.color = "#27ae60";
            });
            var delBtn = document.createElement("button"); delBtn.className = "pbtn red"; delBtn.textContent = "🗑删除";
            delBtn.addEventListener("click", function (e) {
                ...
                pmsgEl.textContent = "🗑 已删除"; pmsgEl.style.color = "#888";
            });
            ...
            /* 全部模式 */
            var selBtn = document.createElement("button"); selBtn.className = "pbtn pink"; selBtn.textContent = "☑ 多选建组";
            selBtn.addEventListener("click", function (e) {
                e.stopPropagation(); selectMode = true; selected = {}; renderAll();
            });
            pbarEl.appendChild(selBtn);
        }
    }
    ...
    function posPanel() {
        var pw = Math.min(window.innerWidth * 0.9, 420), ph = Math.min(window.innerHeight * 0.75, 650), gap = 10;
    ...
    function closeP() { panel.style.display = "none"; panelOpen = false; gsDrop.style.display = "none"; ePop.style.display = "none"; }
    function toggleP() {
        var sel = window.getSelection(); var text = sel ? sel.toString().trim() : "";
        if (panelOpen) { if (text) { sInput.value = text; tabEls[0].click(); doSearch(text); } else closeP(); } else openP(text);
    }
    ...
    function moveTo(x, y) {
        ...
        posX = x; posY = y; fab.style.left = x + "px"; fab.style.top = y + "px"; if (panelOpen) posPanel();
    }
    ...
    fab.addEventListener("touchend", function (e) { e.preventDefault(); e.stopImmediatePropagation(); varwd = dragging, wm = hasMoved; dragging = false; hasMoved = false; if (wd && !wm) setTimeout(function () { toggleP(); }, 50); if (wd && wm) { localStorage.setItem("bnyPosX", String(posX)); localStorage.setItem("bnyPosY", String(posY)); } }, { passive: false });
    ...
    document.addEventListener("mouseup", function () { if (!dragging) return; var wm = hasMoved; dragging = false; hasMoved = false; if (!wm) toggleP(); else { localStorage.setItem("bnyPosX", String(posX)); localStorage.setItem("bnyPosY", String(posY)); } });
