jQuery(() => {
    ...
    var defaultKillPrompt = "请润色以下文本。去除AI模型常见的模板化表达（如\"不禁\"\"宛如\"\"仿佛\"\"微微\"\"轻轻\"\"竟然\"\"不由自主\"等），使文字更自然流畅。保持原文情节、角色、情感和篇幅不变。不要添加或删减内容。直接输出润色后的文本，不要任何解释。";
    var defaultStylePrompt = "请精修以下文本的文风。在保持情节和内容完全不变的前提下，提升文字表现力和文学性。注意节奏、用词和氛围。直接输出修改后的文本，不要任何解释。";
    ...
    getContainer().append(
        ...
        '<div class="inline-drawer-content">' +
            ...
            '<div style="padding:3px 0;"><label style="font-size:12px;color:#888;">API 地址</label>' +
                '<input type="text" id="bny-api-url" placeholder="https://api.openai.com/v1/chat/completions" style="width:100%;padding:5px 8px;border-radius:8px;border:1px solid #ddd;font-size:11px;"/></div>' +
    ...
    function getS() {
        return {
            ...
            model: $("#bny-api-model").val().trim() || "gpt-4o-mini",
            temp: parseFloat($("#bny-temp").val()) || 0.7,
            topp: parseFloat($("#bny-topp").val()) || 1.0,
            killPrompt: $("#bny-kill-prompt").val() || "",
            stylePrompt: $("#bny-style-prompt").val() || ""
        };
    }
    function saveS() {
        localStorage.setItem("bnyS5", JSON.stringify(getS()));
        $("#bny-status").text("✅ 已保存！");
    }
    function loadS() {
        var s = localStorage.getItem("bnyS5");
    ...
    if (prev) {
        ...
        $("#bny-temp").val(prev.temp || 0.7); $("#bny-temp-val").text(prev.temp || 0.7);
        $("#bny-topp").val(prev.topp || 1.0); $("#bny-topp-val").text(prev.topp || 1.0);
    ...
    $("#bny-test-api").on("click", function () {
        ...
        r.html('<span style="color:#888;">🔄 测试中...请求地址: ' + st.url + '</span>');
        fetch(st.url, {
            method: "POST",
        ...
        }).catch(function (err) { r.html('<span style="color:#e74c3c;">❌ ' + err.message + '</span>'); });
    });
    ...
    panel.innerHTML =
        ...
        '<div class="tc on" data-t="search">' +
            '<div class="sh">' +
                ...
                '<button class="btn bgo">搜索</button>' +
                '<button class="btn bcl">清空</button>' +
            '</div>' +
            '<div class="sb">' +
        ...
        '<div class="tc" data-t="polish">' +
            '<div class="pc">' +
                ...
                '<div class="ti"></div>' +
                '<button class="btn bgo" style="width:100%;margin:2px 0;" id="bny-grab">📋 抓取最新回复</button>' +
                '<div class="br">' +
    ...
    var sBtn = panel.querySelector(".bgo");
    var cBtn = panel.querySelector(".bcl");
    ...
    function getLastBot() {
        try { var ctx = SillyTavern.getContext(), c = ctx.chat;
            for (var i = c.length - 1; i >= 0; i--) { if (!c[i].is_user && !c[i].is_system) return { index: i, text: c[i].mes }; }
        } catch (e) {} return null;
    }
    function setAB(d) { killBtn.disabled = d; styleBtn.disabled = d; bothBtn.disabled = d; }

    function doSearch(q) {
        if (!q.trim()) return; q = q.trim();
        var eng = getEngine(); eTag.textContent = eng.name;
    ...
    function posPanel() {
        ...
        var top; if (posY - gap - ph > 5) top = posY - gap - ph;
    ...
    function toggleP() { if (panelOpen) closeP(); else { var sel = window.getSelection(); openP(sel ? sel.toString().trim() : ""); } }
    ...
    grabBtn.addEventListener("click", function (e) {
        ...
        pvEl.textContent = tag + "\n\n" + preview; pvEl.classList.remove("empty");
    ...
    function doPolish(mode) {
        ...
        var prompt, label;
        if (mode === "kill") {
            if (!st.killPrompt) { stEl.textContent = "⚠️ 请填写杀八股指令！"; stEl.className = "st err"; setAB(false); return; }
            prompt = st.killPrompt; label = "杀八股";
        } else if (mode === "style") {
            if (!st.stylePrompt) { stEl.textContent = "⚠️ 请填写文风指令！"; stEl.className = "st err"; setAB(false); return; }
            prompt = st.stylePrompt; label = "文风润色";
        } else {
            if (!st.killPrompt || !st.stylePrompt) { stEl.textContent = "⚠️ 请填写杀八股和文风指令！"; stEl.className = "st err"; setAB(false); return; }
            prompt = st.killPrompt + "\n\n同时也请执行以下要求：\n\n" + st.stylePrompt; label = "杀八股+文风";
        }
    ...
    ubEl.addEventListener("click", function (e) {
        ...
        try {
            ...
            ctx.saveChat(); gFull = undoData.text; var c = exCont(undoData.text); gContent = c || undoData.text;
        } catch (ex) {}
        stEl.textContent = "↩ 已撤销！"; stEl.className = "st"; ubEl.style.display = "none"; rsEl.style.display = "none"; undoData = null;
        pvEl.textContent = gContent.length > 300 ? gContent.substring(0, 300) + "..." : gContent;
    });

    sBtn.addEventListener("click", function (e) { e.stopPropagation(); doSearch(sInput.value); });
    cBtn.addEventListener("click", function (e) { e.stopPropagation(); clearS(); });
    ...
    [killBtn, styleBtn, bothBtn, ubEl, grabBtn].forEach(function (b) {
