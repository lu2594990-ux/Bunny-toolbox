jQuery(() => {
    ...
    function similarity(a, b) {
        ...
        for (var i = 0; i < shorter.length; i++) {
            var idx = longer.indexOf(shorter[i]);
            if (idx !== -1) {
                matches++;
    ...
    panel.innerHTML =
        ...
        '<div class="sel"><label>选择要调试的正则：</label>' +
        '<select id="bny-sel"><option value="">-- 请选择 --</option></select></div>' +
    ...
    function populateSelect() {
        ...
        selEl.innerHTML = '<option value="">-- 请选择（' + cachedRegex.length + '条）--</option>';
    ...
    selEl.addEventListener("change", function (e) {
        e.stopPropagation();
        if (selEl.value === "") {
            contentEl.innerHTML = '<div class="info">👆 请先选择一条正则</div>';
            return;
        }
    ...
    function runAnalysis(idx) {
        ...
        if (!lastMsg) {
            contentEl.innerHTML = '<div class="info">❌ 没有AI消息</div>';
            return;
        }
        ...
        var statusHtml;
        if (directMatch) {
            statusHtml = '<span class="bst ok">✅匹配成功，无需修复</span>';
        } else if (found && found.score >= Math.ceil(found.total * 0.3)) {
            statusHtml = '<span class="bst warn">⚠️ 未匹配，找到相似段落（' + found.score + '/' + found.total + '）</span>';
        } else {
            statusHtml = '<span class="bst no">❌ 未找到相似内容</span>';
        }
        ...
        box1.className = "box";
        box1.innerHTML = '<div class="blabel">📊 ' + statusHtml + '</div>' +
            '<div class="blabel" style="margin-top:6px;">🧩 骨架碎片：</div>';
        ...
        if (fixes.length > 0) {
            ...
            for (var fxi = 0; fxi < fixes.length; fxi++) {
                ...
                frow.appendChild(ar);
                frow.appendChild(rs);
                fixDiv.appendChild(frow);
                fixBox.appendChild(fixDiv);
            }
        ...
        testBtn.addEventListener("click", function (e) {
            ...
            var resDiv = contentEl.querySelector("#bny-result");
            if (!resDiv) {
            ...
            if (ok) {
                ...
                okBox.innerHTML = '<div class="blabel"><span class="bst ok">✅ 匹配成功！</span></div><div class="blabel">替换后预览：</div>';
                ...
                noBox.innerHTML = '<div class="blabel"><span class="bst no">❌仍不匹配</span></div>' +
                    '<div style="font-size:10px;color:#999;padding:4px;">继续调整后再试</div>';
        ...
        saveBtn.addEventListener("click", function (e) {
            ...
            if (saveMsg(lastMsg.idx, newMes)) {
                msgEl.textContent = "✅ 已保存！下滑刷新可看到变化";
            } else {
