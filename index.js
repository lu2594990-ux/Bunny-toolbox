jQuery(() => {
    ...
    styleEl.textContent =
        ...
        ".tab{flex:1;padding:10px 0;text-align:center;font-size:12px;cursor:pointer;color:#aaa;border-bottom:2px solid transparent;transition:.2s;}" +
        ...
        ".cmd-grp-hd:active{background:#f5e8ed;}" +
        ...
        ".cmd-item .ci-edit:active{opacity:.6;}" +
        ".cmd-form{padding:10px 12px;overflow-y:auto;flex:1;}" +
        ".cmd-form label{display:block;font-size:11px;color:#999;margin:8px 0 4px;}" +
        ".cmd-form label:first-child{margin-top:0;}" +
        ".cmd-form input{width:100%;height:34px;border:1px solid #f0d0d8;border-radius:10px;padding:0 12px;font-size:13px;outline:none;background:#fffafc;color:#333;}" +
        ".cmd-form input:focus{border-color:#ff6b9d;}" +
        ...
        ".cmd-form .cf-radio{display:flex;gap:10px;padding:4px 0;}" +
        ".cmd-form .cf-radio label{display:flex;align-items:center;gap:4px;font-size:12px;color:#555;cursor:pointer;margin:0;}" +
        ".cmd-form .cf-var{font-size:10px;color:#aaa;padding:6px 0;line-height:1.6;}";shadow.appendChild(styleEl);
                ...
                if (selectMode || editingGroup !== null) {
                    ...
                    tog.addEventListener("click", function (e) {
                        ...
                        setTimeout(function () {
                            ...
                            if (nowOn) { tog.classList.add("on"); nameEl.className = "pn on-c"; } else { tog.classList.remove("on"); nameEl.className = "pn off-c"; }pmsgEl.textContent = (nowOn ? "✅ 开启" : "❌ 关闭") + " " + name; pmsgEl.style.color = nowOn ? "#27ae60" : "#e74c3c";
    ...
    function renderActions() {
        ...
        if (editingGroup !== null) {
            ...
            cgOn.addEventListener("click", function (e) { e.stopPropagation(); pmsgEl.textContent = "⏳ 批量开启中..."; pmsgEl.style.color = "#888"; batchToggle(cg.ids, true, function () { renderList(); renderBar(); }); });
            var cgOff = mk("button", "pbtn red", "🔌 全关");
            cgOff.addEventListener("click", function (e) { e.stopPropagation(); pmsgEl.textContent = "⏳ 批量关闭中..."; pmsgEl.style.color = "#888"; batchToggle(cg.ids, false, function () { renderList(); renderBar(); }); });
            ...
            abOn.addEventListener("click", function (e) { e.stopPropagation(); pmsgEl.textContent = "⏳ 批量开启中..."; pmsgEl.style.color = "#888"; var ids = getActiveOrder().map(function (x) { return x.identifier; }); batchToggle(ids, true, function () { renderList(); renderBar(); }); });
            var abOff = mk("button", "pbtn red", "🔌 全关");
            abOff.addEventListener("click", function (e) { e.stopPropagation(); pmsgEl.textContent = "⏳ 批量关闭中..."; pmsgEl.style.color = "#888"; var ids = getActiveOrder().map(function (x) { return x.identifier; }); batchToggle(ids, false, function () { renderList(); renderBar(); }); });
            ...
            for (var mi = 0; mi < groups.length; mi++) {
                (function (idx) {
                    ...
                    handle.addEventListener("mousedown", function (e) {
                        e.preventDefault(); e.stopPropagation(); isDrag = true; dragFromIdx = idx; row.classList.add("dragging"); pmsgEl.textContent = "🔀 拖拽中..."; pmsgEl.style.color = "#c44569";var onMM = function (ev) {
                        ...
                        var onMU = function () {
                            ...
                            if (isDrag && dragFromIdx !== null && dragToIdx !== null && dragFromIdx !== dragToIdx) { var gs = loadGroups(); var moved = gs.splice(dragFromIdx, 1)[0]; gs.splice(dragToIdx, 0, moved); saveGroups(gs); if (currentGroup === dragFromIdx) currentGroup = dragToIdx; pmsgEl.textContent = "✅ 已移动"; pmsgEl.style.color = "#27ae60"; }
                            isDrag = false; dragFromIdx = null; dragToIdx = null; renderManage();
                        };
            ...
            for (var di = 0; di < data.length; di++) {
                (function (gIdx) {
                    ...
                    hd.addEventListener("click", function (e) {
                        e.stopPropagation(); cmdOpenGroups[gIdx] = !isOpen; renderCmd();
                    });
                        ...
                        for (var ci = 0; ci < grp.cmds.length; ci++) {
                            (function (cmdIdx) {
                                ...
                                editBtn.addEventListener("click", function (e) {
                                    e.stopPropagation(); cmdEditing = { gIdx: gIdx, cmdIdx: cmdIdx }; cmdFormMode = true; renderCmd();
                                });
    ...
    function renderCmdBar() {
        cmdBarEl.innerHTML = "";
        var addBtn = mk("button", "pbtn pink", "➕ 添加指令"); addBtn.style.flex = "1"; addBtn.style.minWidth = "55px"; addBtn.style.height = "32px"; addBtn.style.border = "none"; addBtn.style.borderRadius = "10px"; addBtn.style.fontSize = "11px"; addBtn.style.cursor = "pointer";
        addBtn.addEventListener("click", function (e) { e.stopPropagation(); cmdEditing = null; cmdFormMode = true; renderCmd(); });
        var mgrBtn = mk("button", "pbtn blue", "⚙️ 管理分组"); mgrBtn.style.flex = "1"; mgrBtn.style.minWidth = "55px"; mgrBtn.style.height = "32px"; mgrBtn.style.border = "none"; mgrBtn.style.borderRadius = "10px"; mgrBtn.style.fontSize = "11px"; mgrBtn.style.cursor = "pointer";
        mgrBtn.addEventListener("click", function (e) {
            ...
            if (action === "1") {
                ...
                var list = "";
                for (var di = 0; di < data.length; di++) { list += (di + 1) + " = " + data[di].name + "\n"; }
                var idx = prompt("选择要删除的分组：\n" + list); if (!idx) return;
                idx = parseInt(idx) - 1;
                if (idx < 0 || idx >= data.length) return;
                if (!confirm("确定删除「" + data[idx].name + "」及其所有指令？")) return;
                data.splice(idx, 1); saveCmdData(data); renderCmd();
                cmdMsgEl.textContent = "✅ 已删除"; cmdMsgEl.style.color = "#27ae60";
            } else if (action === "3") {
                if (data.length === 0) { cmdMsgEl.textContent = "⚠️ 没有分组"; cmdMsgEl.style.color = "#e74c3c"; return; }
                var list2 = "";
                for (var di2 = 0; di2 < data.length; di2++) { list2 += (di2 + 1) + " = " + data[di2].name + "\n"; }
                var idx2 = prompt("选择要重命名的分组：\n" + list2); if (!idx2) return;
                idx2 = parseInt(idx2) - 1;
                ...
                cmdMsgEl.textContent = "✅ 已重命名"; cmdMsgEl.style.color = "#27ae60";
            }
        });
    ...
    function renderCmdForm() {
        ...
        cmdBarEl.innerHTML = "";
        var saveBtn = mk("button", "pbtn green", "✅ 保存"); saveBtn.style.flex = "1"; saveBtn.style.minWidth = "55px"; saveBtn.style.height = "32px"; saveBtn.style.border = "none"; saveBtn.style.borderRadius = "10px"; saveBtn.style.fontSize = "11px"; saveBtn.style.cursor = "pointer";
        saveBtn.addEventListener("click", function (e) {
            ...
            if (isEdit) {
                var oldGIdx = cmdEditing.gIdx;
                var oldCIdx = cmdEditing.cmdIdx;
                if (groupVal === oldGIdx) {
                    freshData[oldGIdx].cmds[oldCIdx] = { name: nameVal, content: contentVal, action: actionVal };
                } else {
                    freshData[oldGIdx].cmds.splice(oldCIdx, 1);
                    freshData[groupVal].cmds.push({ name: nameVal, content: contentVal, action: actionVal });
                }
            } else {
                freshData[groupVal].cmds.push({ name: nameVal, content: contentVal, action: actionVal });
            }
        ...
        var canBtn = mk("button", "pbtn gray", "✖ 取消"); canBtn.style.flex = "1"; canBtn.style.minWidth = "55px"; canBtn.style.height = "32px"; canBtn.style.border = "none"; canBtn.style.borderRadius = "10px"; canBtn.style.fontSize = "11px"; canBtn.style.cursor = "pointer";
        ...
        if (isEdit) {
            var delBtn = mk("button", "pbtn red", "🗑 删除"); delBtn.style.flex = "1"; delBtn.style.minWidth = "55px"; delBtn.style.height = "32px"; delBtn.style.border = "none"; delBtn.style.borderRadius = "10px"; delBtn.style.fontSize = "11px"; delBtn.style.cursor = "pointer";
    ...
    function moveTo(x, y) {
        var mx = window.innerWidth - 52; var my = window.innerHeight - 52;
