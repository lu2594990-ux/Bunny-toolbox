jQuery(() => {
        ...
        '<div class="inline-drawer">' +
            ...
            '<div class="inline-drawer-content">' +
                ...
                '<div style="padding:3px 0;">' +
                    '<label style="font-size:12px;color:#888;">API 地址（填到 /v1 即可，会自动补全）</label>' +
                    '<input type="text" id="bny-api-url" placeholder="https://api.openai.com/v1" style="width:100%;padding:5px 8px;border-radius:8px;border:1px solid #ddd;font-size:11px;" />' +
                '</div>' +
                '<div style="padding:3px 0;">' +
    ...
    $("#bny-temp").val(stLoad.temp);
    $("#bny-temp-val").text(stLoad.temp);
    $("#bny-topp").val(stLoad.topp);
    $("#bny-topp-val").text(stLoad.topp);
    ...
    $("#bny-test-api").on("click", function () {
        ...
        fetch(fullUrl, {
            ...
            if (!res.ok) return res.text().then(function (t) {
                var d = "";
                try { var j = JSON.parse(t); d = j.error ? (j.error.message || JSON.stringify(j.error)) : t; } catch (e) { d = t; }
    ...
    panel.innerHTML =
        ...
        '<div class="bny-tab-content" data-tab="polish">' +
            '<div class="bny-polish-content">' +
                '<div class="bny-section-label">📄 提取的正文：</div>' +
    ...
    var searchBtn = panel.querySelector(".bny-search-header .bny-go");
    ...
    function roughTokenCount(str) { return Math.ceil(str.length / 2); }

    function extractContent(text) {
        var m = text.match(/<content>([\s\S]*?)<\/content>/i);
        return m ? m[1].trim() : null;
    }

    function replaceContent(text, newContent) {
        return text.replace(/<content>[\s\S]*?<\/content>/i, "<content>" + newContent + "</content>");
    }
    ...
    function getLastBotMessage() {
        try {
            ...
            for (var i = chat.length - 1; i >= 0; i--) {
                if (!chat[i].is_user && !chat[i].is_system) return { index: i, text: chat[i].mes };
            }
        } catch (e) {}
        return null;
    }
    ...
    function showError(msg, detail) {
        ...
        if (detail) { errorDetail.textContent = detail; errorDetail.style.display = "block"; }
    ...
    function doSearch(query) {
        ...
        var q = query.trim();
        var eng = getEngine();
        engineTag.textContent = eng.name;
        if (isUrl(q)) {
            var url = toUrl(q);
            iframe.src = url;
            ...
            fallbackLink.href = eng.fallback(q);
        }
    ...
    function callApi(systemPrompt, userContent) {
        var st = loadSettings();
        var fullUrl = buildApiUrl(st.url);
    ...
    function applyResult(newContent) {
        ...
        grabbedContent = newContent;
        var pv = newContent.length > 300 ? newContent.substring(0, 300) + "..." : newContent;
        previewEl.textContent = pv;
        tokenInfo.textContent = "≈ " + roughTokenCount(newContent) + " tokens";
        resultBox.textContent = pv;
    ...
    function doPolish(mode) {
        ...
        if (mode === "kill") {
            ...
            prompt = st.stylePrompt;
            label = "文风润色";
        } else if (mode === "both") {
            if (!st.killPrompt || !st.stylePrompt) { showError("请填写杀八股和文风指令！"); setAllBtns(false); return; }
            prompt = st.killPrompt + "\n\n同时也请执行以下要求：\n\n" + st.stylePrompt;
            label = "杀八股+文风";
        }
        statusText.textContent = "🔄 " + label + "中...";
        statusText.className = "bny-status-text";
        callApi(prompt, grabbedContent).then(function (r) {
            applyResult(r);
            statusText.textContent = "✅ " + label + "完成！";
            statusText.className = "bny-status-text success";
            setAllBtns(false);
        }).catch(function (e) {
            showError(label + "失败", e.message);
            setAllBtns(false);
        });
    }
    ...
    undoBtn.addEventListener("click", function (e) {
        ...
        try {
            ...
            ctx.saveChat();
            grabbedFullText = undoData.text;
            var c = extractContent(undoData.text);
            grabbedContent = c || undoData.text;
        } catch (ex) {}
    ...
    fab.addEventListener("touchend", function (e) {
        ...
        if (wasDragging && !wasMoving) {
            setTimeout(function () { togglePanel(); }, 50);
        }
        if (wasDragging && wasMoving) {
